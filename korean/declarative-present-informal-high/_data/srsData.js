// Configuration
const localStorageKey = "korean" + "declarativePresentInformalHighVerbs" + "Srs";

// Display level
const displayLevel = "SRS Practice";

// Speech Synthesis
const language = "ko-KR";
const activateSpeech = false;

// Gradient
const gradientBackground = "pink-background";
const loadergradientBackground = "pink-loader-background";

// CTA Background
const ctaClass = "pink-cta";

// Exercise Type (character, vocabulary )
const exerciseType = "vocabulary";

// Welcome Popover Content
const welcomeIntro = "In this spaced-repetition practice (SRS), you will need to recall the Declarative Present (Informa High) form of over 100 common verbs. \n \n After your first session, you will also revise learned characters from previous sessions";

const allSyllableMap = {
  "to see/watch": {
    "letter": "봐요"
  },
  "to talk/speak": {
    "letter": "말해요"
  },
  "to ask": {
    "letter": "물어요"
  },
  "to answer": {
    "letter": "대답해요"
  },
  "to think": {
    "letter": "생각해요"
  },
  "to sleep": {
    "letter": "자요"
  },
  "to dream": {
    "letter": "꿈꿔요"
  },
  "to make/create": {
    "letter": "만들어요"
  },
  "to listen/hear": {
    "letter": "들어요"
  },
  "to play": {
    "letter": "놀아요"
  },
  "to need": {
    "letter": "필요해요"
  },
  "to be": {
    "letter": "이에요"
  },
  "to become": {
    "letter": "돼요"
  },
  "to be able to": {
    "letter": "할수있어요"
  },
  "to do": {
    "letter": "해요"
  },
  "to learn": {
    "letter": "배워요"
  },
  "to study": {
    "letter": "공부해요"
  },
  "to write": {
    "letter": "써요"
  },
  "to read": {
    "letter": "읽어요"
  },
  "to work": {
    "letter": "일해요"
  },
  "to teach": {
    "letter": "가르쳐요"
  },
  "to give": {
    "letter": "줘요"
  },
  "to receive": {
    "letter": "받아요"
  },
  "to clean": {
    "letter": "청소해요"
  },
  "to wash": {
    "letter": "씻어요"
  },
  "to call (telephone)": {
    "letter": "전화해요"
  },
  "to have/exist": {
    "letter": "있어요"
  },
  "to not have": {
    "letter": "없어요"
  },
  "to finish": {
    "letter": "끝나요"
  },
  "to know": {
    "letter": "알아요"
  },
  "to not know": {
    "letter": "몰라요"
  },
  "to practice": {
    "letter": "연습해요"
  },
  "to take (a photo)": {
    "letter": "찍어요"
  },
  "to take/carry": {
    "letter": "가져가요"
  },
  "to draw/paint": {
    "letter": "그려요"
  },
  "to put": {
    "letter": "놓아요"
  },
  "to come": {
    "letter": "와요"
  },
  "to go": {
    "letter": "가요"
  },
  "to walk": {
    "letter": "걸어요"
  },
  "to sit": {
    "letter": "앉아요"
  },
  "to wake up": {
    "letter": "일어나요"
  },
  "to dance": {
    "letter": "춤춰요"
  },
  "to meet": {
    "letter": "만나요"
  },
  "to exercise": {
    "letter": "운동해요"
  },
  "to drive": {
    "letter": "운전해요"
  },
  "to ride": {
    "letter": "타요"
  },
  "to open": {
    "letter": "열어요"
  },
  "to close": {
    "letter": "닫아요"
  },
  "to arrive": {
    "letter": "도착해요"
  },
  "to depart": {
    "letter": "출발해요"
  },
  "to enter": {
    "letter": "들어와요"
  },
  "to exit": {
    "letter": "나가요"
  },
  "to wait": {
    "letter": "기다려요"
  },
  "to stop": {
    "letter": "그만해요"
  },
  "to hurry": {
    "letter": "서둘러요"
  },
  "to move": {
    "letter": "움직이에요"
  },
  "to return": {
    "letter": "돌아와요"
  },
  "to travel": {
    "letter": "여행해요"
  },
  "to eat": {
    "letter": "먹어요"
  },
  "to drink": {
    "letter": "마셔요"
  },
  "to cook": {
    "letter": "요리해요"
  },
  "to chop": {
    "letter": "썰어요"
  },
  "to boil": {
    "letter": "끓여요"
  },
  "to fry": {
    "letter": "볶아요"
  },
  "to stir": {
    "letter": "휘저어요"
  },
  "to grill/bake/roast": {
    "letter": "구워요"
  },
  "to mix": {
    "letter": "섞어요"
  },
  "to pour": {
    "letter": "부어요"
  },
  "to wear (a hat or glasses)": {
    "letter": "써요"
  },
  "to wear (shirt, pants, dress)": {
    "letter": "입어요"
  },
  "to wear (shoes or socks)": {
    "letter": "신어요"
  },
  "to undress": {
    "letter": "벗어요"
  },
  "to change clothes": {
    "letter": "갈아입어요"
  },
  "to cry": {
    "letter": "울어요"
  },
  "to like": {
    "letter": "좋아해요"
  },
  "to dislike": {
    "letter": "싫어해요"
  },
  "to laugh": {
    "letter": "웃어요"
  },
  "to be happy": {
    "letter": "기뻐해요"
  },
  "to love": {
    "letter": "사랑해요"
  },
  "to fall in love": {
    "letter": "사랑해요"
  },
  "to promise": {
    "letter": "약속해요"
  },
  "to confess": {
    "letter": "고백해요"
  },
  "to worry": {
    "letter": "걱정해요"
  },
  "to marry": {
    "letter": "결혼해요"
  },
  "to lie": {
    "letter": "거짓말해요"
  },
  "to live": {
    "letter": "살아요"
  },
  "to die": {
    "letter": "죽어요"
  },
  "to fight/argue": {
    "letter": "싸워요"
  },
  "to win": {
    "letter": "이겨요"
  },
  "to lose": {
    "letter": "이겨요"
  },
  "to be sorry": {
    "letter": "죄송해요"
  },
  "to remember": {
    "letter": "기억해요"
  },
  "to decide": {
    "letter": "결정해요"
  },
  "to buy": {
    "letter": "사요"
  },
  "to sell": {
    "letter": "팔아요"
  },
  "to order": {
    "letter": "주문해요"
  },
  "to help": {
    "letter": "도와줘요"
  },
  "to pay": {
    "letter": "내요"
  },
  "to send": {
    "letter": "보내요"
  },
  "to borrow": {
    "letter": "빌려요"
  },
  "to use": {
    "letter": "사용해요"
  }
}

function translation(n) {
    if (!n) { n = this; }
    console.log(n);
    let letterObj = allSyllableMap[n];
    console.log(letterObj);
    return letterObj.letter;
}

String.prototype.romanize = translation;
